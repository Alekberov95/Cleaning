...
// === Основная функция перевода (с поддержкой placeholder) ===
function applyLang(lang) {
    // Защита от несуществующего языка
    if (!translations[lang]) {
        console.warn(`Язык "${lang}" не найден, используется "ru"`);
        lang = "ru";
    }
    ...
    // Переводим обычные элементы с data-i18n
    ...
    // Событие — можно использовать в других скриптах
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    document.querySelectorAll(".lang-switch button").forEach(btn => {
        ...
        // Подсвечиваем активную кнопку при загрузке
